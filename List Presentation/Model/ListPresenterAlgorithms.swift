...
func findToggledListItems(initialListItems: [ListItem], changedListItems: [ListItem]) -> [ListItem] {
    return changedListItems.filter { changedListItem in
        if let indexOfChangedListItemInInitialListItems = initialListItems.index(of: changedListItem) {
...
*/
func replaceAnyEqualUnchangedNewListItemsWithPreviousUnchangedListItems( replaceableNewListItems: inout [ListItem], previousUnchangedListItems: [ListItem]) {
    let replaceableNewListItemsCopy = replaceableNewListItems
    
    for (idx, replaceableNewListItem) in replaceableNewListItemsCopy.enumerated() {
        if let indexOfUnchangedListItem = previousUnchangedListItems.index(of: replaceableNewListItem) {
