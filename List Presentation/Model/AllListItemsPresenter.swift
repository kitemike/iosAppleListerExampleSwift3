...
/**
    ...
    being updated.
*/
final  class AllListItemsPresenter: NSObject, ListPresenterType {
    // MARK: Properties
    ...
    */
    ...
    // MARK: ListPresenterType
     ...
     var color: List.Color {
        ...
        set {
            updateListItemsWithRawColor(rawColor: newValue.rawValue)
        }
    }
    
     var archiveableList: List {
    ...
    /**
        ...
        undo manager.
    */
     func setList(list newList: List) {
        /**
        ...
        */
        if isInitialList {
            ...
            list = newList
            list.items = reorderedListItemsFromListItems(listItems: list.items)

            delegate?.listPresenterDidRefreshCompleteLayout(listPresenter: self)
            
            return
        }
        
        // called whenever a document.load() is triggered. Oddly, when rotated to fill the DetailView, rather than leaving blank.
        ...
        */
        if listItemsBatchChangeKind == nil {
            if oldList.color != newList.color {
                ...
                updateListColorForListPresenterIfDifferent(listPresenter: self, color: &list.color, newColor: newList.color, isForInitialLayout: true)
            }
        ...
        */
        ...
        delegate?.listPresenterWillChangeListLayout(listPresenter: self, isInitialLayout: true)
        
        // Make the changes based on the unique change kind.
        switch listItemsBatchChangeKind! {
            case .Removed:
                removeListItemsFromListItemsWithListPresenter(listPresenter: self, initialListItems: &list.items, listItemsToRemove: newRemovedListItems)
            
            case .Inserted:
                unsafeInsertListItem(listItem: newInsertedListItems.first!)
            ...
            case .Toggled:
                ...
                unsafeToggleListItem(listItem: listItemToToggle)

            case .UpdatedText:
                updateListItemsWithListItemsForListPresenter(listPresenter: self, presentedListItems: &list.items, newUpdatedListItems: newListItemsWithUpdatedText)

            default:
                break
        }
        
        updateListColorForListPresenterIfDifferent(listPresenter: self, color: &list.color, newColor: newList.color)
        
        delegate?.listPresenterDidChangeListLayout(listPresenter: self, isInitialLayout: true)
    }
    ...
    */
     func insertListItem(listItem: ListItem) {
        delegate?.listPresenterWillChangeListLayout(listPresenter: self, isInitialLayout: false)
        
        unsafeInsertListItem(listItem: listItem)
        
        delegate?.listPresenterDidChangeListLayout(listPresenter: self, isInitialLayout: false)
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
     func insertListItems(listItems: [ListItem]) {
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
    @objc  func removeListItem(listItem: ListItem) {
        ...
        delegate?.listPresenter(listPresenter:self, didRemoveListItem: listItem, atIndex: listItemIndex!)
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
    @objc  func removeListItems(listItems: [ListItem]) {
        if listItems.isEmpty { return }
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
    @objc  func updateListItem(listItem: ListItem, withText newText: String) {
        ...
        delegate?.listPresenter(listPresenter: self, didUpdateListItem: listItem, atIndex: index)
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
     func canMoveListItem(listItem: ListItem, toIndex: Int) -> Bool {
    ...
    */
    @objc  func moveListItem(listItem: ListItem, toIndex: Int) {
        precondition(canMoveListItem(listItem: listItem, toIndex: toIndex), "An item can only be moved if it passes a \"can move\" test.")
        
        delegate?.listPresenterWillChangeListLayout(listPresenter: self, isInitialLayout: false)

        let fromIndex = unsafeMoveListItem(listItem: listItem, toIndex: toIndex)
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
     func toggleListItem(listItem: ListItem) {
        delegate?.listPresenterWillChangeListLayout(listPresenter: self, isInitialLayout: false)
        
        let fromIndex = unsafeToggleListItem(listItem: listItem)
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
     func updatePresentedListItemsToCompletionState(completionState: Bool) {
        ...
        toggleListItemsWithoutMoving(listItems: presentedListItemsNotMatchingCompletionState, undoActionName: undoActionName)
    }
    ...
    */
     func listItemsAtIndexes(indexes: NSIndexSet) -> [ListItem] {
    ...
    */
    @objc private func toggleListItemForUndo(listItem: ListItem, toPreviousIndex previousIndex: Int) {
        ...
        // Move the list item.
        let fromIndex = unsafeMoveListItem(listItem: listItem, toIndex: previousIndex)
        
        // Update the list item's state.
        listItem.isComplete = !listItem.isComplete
        
        delegate?.listPresenter(listPresenter: self, didUpdateListItem: listItem, atIndex: previousIndex)
    ...
    */
    @objc private func insertListItemsForUndo(listItems: [ListItem], atIndexes indexes: [Int]) {
        ...
        for (listItemIndex, listItem) in listItems.enumerated() {
            ...
            delegate?.listPresenter(listPresenter: self, didInsertListItem: listItem, atIndex: insertionIndex)
        }
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
    @objc private func updateListItemsWithRawColor(rawColor: Int) {
        ...
        updateListColorForListPresenterIfDifferent(listPresenter: self, color: &list.color, newColor: newColor, isForInitialLayout: false)
    ...
    */
    @objc private func toggleListItemsWithoutMoving(listItems: [ListItem], undoActionName: String) {
        ...
        for listItem in listItems {
            ...
            delegate?.listPresenter(listPresenter: self, didUpdateListItem: listItem, atIndex: updatedIndex)
        }
...
//        undoManager?.setActionName(undoActionName)
    ...
    */
    private func unsafeInsertListItem(listItem: ListItem) {
        ...
        delegate?.listPresenter(listPresenter: self, didInsertListItem: listItem, atIndex: indexToInsertListItem)
    }
    ...
    */
    private func unsafeMoveListItem(listItem: ListItem, toIndex: Int) -> Int {
        ...
        delegate?.listPresenter(listPresenter: self, didMoveListItem: listItem, fromIndex: fromIndex, toIndex: toIndex)
    ...
    private func unsafeToggleListItem(listItem: ListItem) -> Int {
        ...
        let fromIndex = unsafeMoveListItem(listItem: listItem, toIndex: targetIndex)
        
        // Update the list item's state.
        listItem.isComplete = !listItem.isComplete
        delegate?.listPresenter(listPresenter: self, didUpdateListItem: listItem, atIndex: targetIndex)
