...
//    let appDelegateQueue = dispatch_queue_create("com.example.apple-samplecode.lister.appdelegate", DISPATCH_QUEUE_SERIAL)
    let appDelegateQueue = DispatchQueue(label: "com.example.apple-samplecode.lister.appdelegate")
...
/*     defer
 ...
 let textAttributes = listViewController.textAttributes
    ...
    func configureListsController(storageOptionChangeHandler: ((Void) -> Void)? = nil) {
        
        if listsController == nil {
            
            // There is currently no lists controller. Configure an appropriate one for the current configuration.
            listsController = AppConfiguration.sharedConfiguration.listsControllerForCurrentConfigurationWithPathExtension(pathExtension: AppConfiguration.appFileExtension,
                                                                                                                           firstQueryHandler: storageOptionChangeHandler)
            
            // Ensure that this controller is passed along to the `ListDocumentsViewController`.
            if splitViewController.viewControllers.count == 0 {
                
                assertionFailure() // this has triggered after editing the storyboard
                return
            }
            
            let master = splitViewController.viewControllers.first as! UINavigationController
            
            if let listDocumentsViewController = master.viewControllers.first as? ListDocumentsViewController {
