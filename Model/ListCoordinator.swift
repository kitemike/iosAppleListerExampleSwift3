...
/**
    ...
    different `URL` instances that are important. For example, in Lister there are two types of
    ...
    Instances of `ListCoordinator` can search for URLs in an asynchronous way. When a new `URL`
    instance is found, removed, or updated, the `ListCoordinator` instance must make its delegate aware
    of the updates. If a failure occured in removing or creating an `URL` for a given list, it must
...
*/
 protocol ListCoordinator: class {
    ...
    */
    ...
    /**
        Starts observing changes to the important `URL` instances. For example, if a `ListCoordinator`
    ...
    */
    ...
    /**
        Stops observing changes to the important `URL` instances. For example, if a `ListCoordinator`
    ...
    */
    ...
    /**
        Removes `URL` from the list of tracked `URL` instances. For example, an iCloud-specific
        ...
        with the removed `URL`. If a failure occurs when removing `URL`, the coordinator object is
        ...
        - parameter URL: The `URL` instance to remove from the list of important instances.
    */
    func removeListAtURL(URL: URL)

    /**
        Creates an `URL` object representing `list` with the provided name. Callers of this method
        ...
        tracked `URL` as an inserted URL. If the creation was not successful, this method should 
        ...
        - parameter list: The list to create a backing `URL` for.
...
/**
    ...
    tracked `URL` instances. The `ListCoordinatorDelegate` also allows a `ListCoordinator` to notify
 ...
 protocol ListCoordinatorDelegate: class {
    ...
    */
    ...
    /**
        ...
        tried to remove a specific URL from the tracked `URL` instances. For more information about
    ...
    */
    ...
    /**
        ...
        - parameter URL: The `URL` instance that couldn't be created for a list.
